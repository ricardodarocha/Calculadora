...
begin
  ...
  if FParser.Parent.DoGetVariable(FLex.Str, Value) then
    ...
    raise EParserError.Create('A variável ' + FLex.Str + ' não foi informada.');
end;
...
begin
  ...
  begin
    ...
    if FErrorMessage = 'Variant or safe array index out of bounds' then
      fErrorMessage := 'Função requer mais parâmetros, separados por ";"';
